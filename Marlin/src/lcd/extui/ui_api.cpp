...
namespace ExtUI {
  ...
  celsius_t getTargetTemp_celsius(const extruder_t extruder) {
    ...
    // Return the configured bed temperature for preheating before leveling.
    // Many ExtUI implementations simply return LEVELING_BED_TEMP; provide
    // a default implementation here so the symbol is available even when
    // individual ExtUI implementation files are excluded from the build.
